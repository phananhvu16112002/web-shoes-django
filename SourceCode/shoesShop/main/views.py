...
def product_list(request):
    ...
    # min_price = ProductAttribute.objects.aaggregate(Min('Price'))
    # max_price = ProductAttribute.objects.aaggregate(Max('Price'))
    
    # 'min_price':min_price,'max_price':max_price
    
    return render(request,'product_list.html',{'data':data,'total_data':total_data,}) 

#Product acording to Category
...
def cart_list(request):
    ...
    if 'cartdata' in request.session:
        for p_id,item in request.session['cartdata'].items():
            temp = item['price'].strip('$')
            total_amt += int(item['qty']) * float(temp)
...
def checkout(request):
    ...
    if 'cartdata' in request.session:
        for p_id,item in request.session['cartdata'].items():
            temp = item['price'].strip('$')
            totalAmt += int(item['qty']) * float(temp)
        #Order
